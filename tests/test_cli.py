...
def test_cli_baseline_version_gcs_store_loads(tmp_path, monkeypatch):
    current = tmp_path / 'c.csv'
    pd.DataFrame({'x': [10, 11, 12, 13, 14]}).to_csv(current, index=False)
    ...
    monkeypatch.setattr(cli_mod, "GCSBaselineStore", _FakeGCSStore)
    runner = CliRunner()
    result = runner.invoke(
        check,
        [
...
def test_cli_baseline_version_gcs_requires_bucket(tmp_path):
    current = tmp_path / 'c.csv'
    pd.DataFrame({'x': [10, 11, 12]}).to_csv(current, index=False)
    runner = CliRunner()
    result = runner.invoke(
...
def test_cli_baseline_version_azure_store_loads(tmp_path, monkeypatch):
    current = tmp_path / 'c.csv'
    pd.DataFrame({'x': [10, 11, 12, 13, 14]}).to_csv(current, index=False)
    ...
    monkeypatch.setattr(cli_mod, "AzureBlobBaselineStore", _FakeAzureStore)
    runner = CliRunner()
    result = runner.invoke(
        check,
        [
            ...
            '--baseline-container', 'my-container',
            '--baseline-prefix', 'my-prefix',
            '--current', str(current),
            '--output-json',
        ],
...
def test_cli_baseline_version_azure_requires_container(tmp_path):
    current = tmp_path / 'c.csv'
    pd.DataFrame({'x': [10, 11, 12]}).to_csv(current, index=False)
    runner = CliRunner()
    result = runner.invoke(
