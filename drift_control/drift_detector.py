...
class DataDriftDetector:
    ...
    def calculate_drift(self, steps: int = 100) -> dict:
    ...
    def plot_categorical_to_numeric(
        ...
        g.map(sns.violinplot, hue=plot_df['_source'], split=True, **plot_kws)
        g.add_legend()
    ...
    def _rmse(self, targets: np.ndarray, predictions: np.ndarray) -> float:
    ...
    def compare_ml_efficacy(
            ...
            high_cardinality_columns: list = None,
            OHE_columns_cutoff: int = 5,
            train_size: float = 0.7,
            model_prior: object = None,
            model_post: object = None
            ) -> pd.DataFrame:
    ...
    def _ml_data_prep(self):
        ...
        if self.test_data is None:
            msg = (
                ...
                f"a {round(self.train_size * 100, 0)}-{round((1 - self.train_size) * 100, 0)} shuffle split from the post data set."
            )
            logger.info(msg)
        ...
        OHE_columns = [col for col in self.OHE_columns if col != self.target_column]
        high_cardinality_columns = [col for col in self.high_cardinality_columns if col != self.target_column]
        ...
        if len(high_cardinality_columns) > 0:
            ...
            train_prior[high_cardinality_columns] = tf_prior.fit_transform(train_prior[high_cardinality_columns], train_prior[self.target_column])
            test_prior[high_cardinality_columns] = tf_prior.transform(test_prior[high_cardinality_columns], test_prior[self.target_column])
            train_post[high_cardinality_columns] = tf_post.fit_transform(train_post[high_cardinality_columns], train_post[self.target_column])
            test_post[high_cardinality_columns] = tf_post.transform(test_post[high_cardinality_columns], test_post[self.target_column])
