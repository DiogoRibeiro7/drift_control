...
class GCSBaselineStore:
    def __init__(
        ...
        if gcs_client is None:
            try:
                ...
                raise RuntimeError("google-cloud-storage is required for GCSBaselineStore") from exc
            gcs_client = storage.Client()
        self.bucket = gcs_client.bucket(self.bucket_name)
    ...
    def save(
        ...
        payload = buf.getvalue()
        self.bucket.blob(key).upload_from_string(payload)
        meta = {
            "name": name,
            "version": version,
            "path": f"gs://{self.bucket_name}/{key}",
            "format": selected,
            "timestamp_utc": datetime.now(timezone.utc).isoformat(),
            "row_count": int(len(data)),
            "dataset_sha256": self._bytes_sha256(payload),
        ...
        self.bucket.blob(self._meta_key(name, version, fmt=selected)).upload_from_string(json.dumps(meta).encode("utf-8"))
        return meta["path"]
